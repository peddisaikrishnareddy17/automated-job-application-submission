...
(async () => {
    try {
        ...
        console.log('4. Determining target job...');
        const customUrl = process.argv[2];
        let targetJobData;

        if (customUrl) {
            console.log(`-> Using custom provided URL: ${customUrl}`);
            targetJobData = {
                title: 'User Selected Job',
                company: 'Unknown',
                location: 'Remote',
                apply_url: customUrl,
                portal: 'generic' // use the generic ai parser
            };
        } else {
            console.log('-> No URL provided. Fetching from APIs (Arbeitnow & RemoteOK)...');
            ...
            if (apiJobs.length === 0) {
                ...
                apiJobs.push({ title: 'Synthetic Dev', company: 'Mock Inc', location: 'Remote', apply_url: 'https://example.com', portal: 'arbeitnow' });
            }
            
            targetJobData = apiJobs[0];
            console.log(`-> Using job: ${targetJobData.title} at ${targetJobData.company}`);
        }
        
        console.log('5. Creating database Job entry...');
        targetJobData.url = targetJobData.apply_url;
        targetJobData.portal = targetJobData.portal || 'generic'; 
